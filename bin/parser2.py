		...
		if filename != "":
			...
			for line in infile:
				...
				if (runParsed == None):
					...
					if i>=((34+(runParsed*(maxCycles+6)))-(maxCycles+6)) and i<=((34+(runParsed*(maxCycles+6)))-7):
						#print('badass number is: {}', ((33+(runParsed*(maxCycles+6)))-(maxCycles+6)))
						if rpl_values!=None:
...
def plot_image(parsed_data,filenumber, column):
		...
		#looks like there is only one set of data in the list!
		...
		#data_dict=parsed_data[filenumber] #original
...
if __name__ == '__main__':
	...
	if len(sys.argv) >= 2:
		...
		if sys.argv[1] == 'parse':
			...
			#to omit the last empty dictionary inside parsed_data list:
			del parsed_data[-1]
			del parsed_data[0]
