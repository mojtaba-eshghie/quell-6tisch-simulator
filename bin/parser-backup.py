...
def parse(filename):
	...
	command="dir "
	args=r'{} /b /s /A-D /o:gn'.format(filename)
	output = os.popen(command+args)
	the_command_out = output.read().split('\n')
	print(the_command_out)



	for line in the_command_out:
		print('line#{0}: {1}'.format(the_command_out.index(line), line))
	...
	print(tmp) #if is empty, then we have problems!
	if tmp != True:
		print('some problem with tmp')
	print("this is the command used: {}".format(command))
	print("this is the args used: {}".format(args))
	...
	if filename != "":
		print(filename.split("_"));print "Filtering: "+str(filename)
		...
		for line in infile:
			...
			print "working on line#: {}".format(i.__str__())
			if i==24:
	...
	#print data_dict
	if filename == "":
			print("couldn't file the file actually!")
	print "End parsing"
...
if __name__ == '__main__':
	...
	if sys.argv[1] == 'parse':
		...
		for i in range(len(sys.argv)-2):
			...
			if sys.argv[i+2][len(sys.argv[i+2])-1]=='/':  #check.log no / is in the name
