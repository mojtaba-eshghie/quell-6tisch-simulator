...
STOCASTICAL_PARENT_SWITCH = True
...
class Mote(object):
    ...
    RPL_PARENT_SET_SIZE                = 3 #formerly one!
    ...
    def __init__(self,id):
        ...
        self.queue_footprint = list()

        self.smoother = 0.2
        self.power = 1.1 #used to be 1.1
    ...
    #==== Sampling & Research Layer
    '''
    All the interval stuff and samplings must be defined here.
    '''
    ...
    def _schedule_sampling(self):
        ...
        with self.dataLock:
            ...
            cycle = int(math.ceil(SAMPLING_PERIOD / (self.settings.slotframeLength * self.settings.slotDuration)))
            ...
            self.engine.scheduleAtAsn(
                ...
                cb=self._do_sampling,
                uniqueTag=(self.id, '_do_sampling'),
                priority=3,
            )
    ...
    def _do_sampling(self):
        ...
        with self.dataLock:
            self.queue_footprint.append(self.txQueue.__len__())
        ...
        with self.dataLock:
            ...
            for (neighbor,neighborRank) in self.neighborRank.items():
                ...
                if rankIncrease!=None and rankIncrease<=min([self.RPL_MAX_RANK_INCREASE, self.RPL_MAX_TOTAL_RANK-neighborRank]): 
                    ...
                    #check.log if there is a loop and if exists, skip the neighbor
		    rootReached=False
		    skipNeighbor=False
	            inode=neighbor
                    ...
                    for (mote,rank) in sorted_potentialRanks[:self.RPL_PARENT_SET_SIZE]:
                        ...
                        if mote == self.preferredParent:                      
                            ...
                            if rank-newrank<self.RPL_PARENT_SWITCH_THRESHOLD:
                                # parent switch process
                                if STOCASTICAL_PARENT_SWITCH and (self.engine.objective_function == 'quell'):
                                    randoms = list(np.random.choice(2, 1, p=[0.3, 0.7]))
                                    if randoms[0] == 0:
                                        pass
                                    else:
    ...
    def _rpl_calcRankIncrease(self, neighbor):
        ...
        with self.dataLock:
            ...
            if self.engine.objective_function == 'OF0':
                ...
                '''                
                if neighbor.txQueue.__len__() == self.TSCH_QUEUE_SIZE:
                    #neighbor queue is full, lets increase its rank via our new algorithm
                    return int(((3 * etx - 2) * (1 - self.smoother) + (neighbor.txQueue.__len__() ** self.power) * (self.smoother)) * self.RPL_MIN_HOP_RANK_INCREASE)
                else:
                    return int((3 * etx - 2) * self.RPL_MIN_HOP_RANK_INCREASE)
                '''

                randoms = list(np.random.choice(2, 1, p=[0.7, 0.3]))

                if randoms[0] == 0:
                    if neighbor.txQueue.__len__ == 0:
                        scaled_len = 0
                    else:
                        #scaled_len = (neighbor.txQueue.__len__() * 0.03) + 1
                        scaled_len = (neighbor.txQueue.__len__() * 0.03)
                    #return int(((3 * etx - 2) + (scaled_len ** self.power)) * self.RPL_MIN_HOP_RANK_INCREASE)
                    #return int(((3 * etx - 2) + scaled_len) * self.RPL_MIN_HOP_RANK_INCREASE)
                    return int((3 * (etx + scaled_len) - 2) * self.RPL_MIN_HOP_RANK_INCREASE)
                    ...
                    #return int(((3 * etx - 2) * (1 - self.smoother) + (neighbor.txQueue.__len__() ** self.power) * (self.smoother)) * self.RPL_MIN_HOP_RANK_INCREASE)
                    #q = (neighbor.txQueue.__len__() / 10) * 2
                    #return int((3 * etx - 2 + q) * self.RPL_MIN_HOP_RANK_INCREASE)
                else:
                    # the OF0
                    return int((3 * etx - 2) * self.RPL_MIN_HOP_RANK_INCREASE)
                ...
                '''
                print('my rank is {0}'.format(self.rank))
                #lets bypass quell and use the OF0
                return int((3 * etx - 2) * self.RPL_MIN_HOP_RANK_INCREASE)
                '''
    ...
    def _sixtop_rxhousekeeping_per_neighbor(self,neighbor):
        ...
        for (ts,ch),cell in rxCells:
            ...
            # relocate: add new first
            #bug! before remove, it is necessary to check.log if a new cell has been really reserved, this happens in all calls to _sixtop_cell_deletion_sender
    ...
    def _estimateETX(self,neighbor):
        ...
        with self.dataLock:
            ...
            #print('this is me: {0}'.format(self.id))
            #print('this is my neighbor: {0}\n'.format(neighbor.id))
