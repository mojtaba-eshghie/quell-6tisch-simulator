...
class SimEngine(threading.Thread):
	...
	print "Drops by propagation "+str(self.dropByPropagation)
        ...
        motes_footprint_dict = dict()
        for mote in self.motes:
            motes_footprint_dict[mote.id] = mote.queue_footprint

        with open('motes_footprint_file.json', 'w') as motes_footprint_file:
            json.dump(motes_footprint_dict, motes_footprint_file)

        if not Topology.Check.CHECK:
            with open('topo.json', 'w') as topo:
                for mote in self.motes:
                    location_dictionary[mote.id] = [mote.x, mote.y]
                json.dump(location_dictionary, topo)
                #print(location_dictionary)
                #print(Topology.MR.rssis)

            with open('rssi.json', 'w') as rs:
                rs.write(str(Topology.MR.rssis))



        if Topology.Check.SAVE_EDGES:
            e_dict = dict()
            with open('edges.json', 'w') as edges:
                for mote in self.motes:
                    if mote.preferredParent:
                        #if mote.id in e_dict.keys():
                        #   e_dict[mote.id].append()
                        e_dict[mote.id] = mote.preferredParent.id
                json.dump(e_dict, edges)
