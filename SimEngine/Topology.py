...
class Check(object):
    #if you wanna save rssi and locations set this to False
    CHECK = True
    #if you wanna save edges set this to True
    SAVE_EDGES = False

class MR:
    rssis = dict()
    if Check.CHECK:
        with open('C:\\Users\\mojtaba\\Desktop\\fresh\\bin\\rssi.json', 'r') as rssi:
            string = rssi.read()
            lister = string.split(', (')
            for element in lister:
                ...
                p2 = p2.replace('}', '')
                rss = rss.replace('}', '')
                rssis[(int(p1), int(p2))] = float(rss)
                # print('{0}, {1}: {2}'.format(p1, p2, rss))
...
class Topology(object):
    ...
    def createTopology(self):
        ...
        if Check.CHECK:
            import json
            file = open('C:\\Users\\mojtaba\\Desktop\\fresh\\bin\\topo.json', 'r')
            mote_locations = json.load(file)
            new_mote_locations = dict()
            for key, value in mote_locations.items():
                new_mote_locations[int(key)] = value
        ...
        for mote in self.motes:
            ...
            while not connected:
                ...
                if Check.CHECK:
                    mote.setLocation(
                        x=new_mote_locations[mote.id][0],
                        y=new_mote_locations[mote.id][1]
                    )
                else:
                    mote.setLocation(
                        x=self.settings.squareSide * random.random(),
    ...
    def _computeRSSI(self,mote,neighbor):
        ...
        #print('rssi between {0} and {1} is: {2}'.format(mote.id, neighbor.id, rssi))
        #MR.rssis[(mote.id, neighbor.id)] = rssi
    ...
    def _computeRSSI(self,mote,neighbor):
        ...
        if not Check.CHECK:
            ...
            rssi = MR.rssis[(mote.id, neighbor.id)]
            return rssi
